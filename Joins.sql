...
--1) INNER JOIN
--Ortak company'ler için company_name, order_id ve order_date değerlerini cağırın.
SELECT company_name, order_id, order_date
FROM my_companies INNER JOIN orders 
ON my_companies.company_id = orders.company_id ; --2 table daki ortak data lar id ler oldugu icin 
--2 table i id'ler sayesinde connect kuruyoruz

-- 2) LEFT JOIN
--my_companies table'ındaki companyler için order_id ve order_date değerlerini cağırın.
SELECT mc.company_name, o.order_id, o.order_date
FROM my_companies mc LEFT JOIN orders o
ON mc.company_id= o.company_id ;

--3) RIGHT JOIN
--Orders table'ındaki company'ler için company_name, company_id ve order_date değerlerini cağırın.
