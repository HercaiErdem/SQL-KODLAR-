...
SELECT urun_adi, min(urun_miktari) AS min_urun_mik,
                 max (urun_miktari)AS max_urun_mik from manav
WHERE urun_adi is not null
group by urun_adi
...
SELECT isim , urun_adi, sum(urun_miktari) as urunlerin_top from manav
...
-- * ==> her ikisi icinde gruplandir demek


-- Her ulke için bay ve bayan çalışan sayısı 
--ve yaş ortalamasını 
--ve maası 30000 den büyük olanları sorgulayınız.
SELECT ulke, cinsiyet, round(avg(yas))AS yas_ort, count(*) from personel
where maas >30000
group by ulke, cinsiyet;
